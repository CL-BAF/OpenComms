...
- `save(state)` (`src/core/store.ts`): `mkdirSync(recursive)`, `writeFileSync(tmp, payload, { flush: true })` where tmp = `.state.<pid>.<rand>.tmp`, `renameSync(tmp, file)`. On replacement failure: retry after bounded 50ms and 100ms blocking waits. If all three attempts fail, throw a persistence error with the original cause and preserve the previous file. Temporary files are cleaned up best-effort. Both stores share `replaceStateFile` in `src/core/atomic-file.ts`.
