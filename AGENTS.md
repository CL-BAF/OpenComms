...
- 4 source files: `src/types.ts` (187), `src/store.ts` (110), `src/engine.ts` (576), `src/plugin.ts` (496)
...
| `src/engine.ts:1` | Pure deterministic business logic (14 exports) | Changing channel/message/queue logic, validation |
...
// Channel: src/types.ts:60 — id, name, project_id, worktree, paused, members[≤2], rate, cooldown_until, seen_content, max_hops=4, rate_limit=20/min, stale_event_ms=5min
...
Tests in `test/unit/engine.test.ts` (434 LOC, 27 tests), `store.test.ts` (67 LOC, 3 tests), and `test/live/live.test.ts` (guarded live flow).
...
3. **Don't guess tool args** — `docs/TOOLS_AND_COMMANDS.md` has exact schemas for all 10 tools.
